...
gotADir:
    ...
    if (StrStrW(lpCommandLine, L"--slient") == NULL) {
        BROWSEINFO bi;
        ZeroMemory(&bi, sizeof(BROWSEINFO));
        LPMALLOC pMalloc;

        bi.ulFlags = BIF_NEWDIALOGSTYLE;
        bi.lpszTitle = _T("Select a folder to install XL");
        ...
        {
            MessageBox(NULL, TEXT("Select Emply!"), TEXT("XL Setup"), MB_OK);
            ExitProcess(-2);
        }

        swprintf_s(cmd, L"\"%s\" --install . --installationPath=\"%s\" %s", updateExePath, path, lpCommandLine);
    }
    else {
